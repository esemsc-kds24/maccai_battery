...
def parse_qe_final_structure(output: str) -> Optional[dict]:
    """Parse the final relaxed structure from a QE pw.x relax output.

    Extracts the lattice vectors (from crystal axes + alat) and the
    final ATOMIC_POSITIONS block between 'Begin final coordinates' and
    'End final coordinates'.

    Returns
    -------
    dict or None
        pymatgen Structure serialized as a dict via structure.as_dict(),
        or None if the output doesn't contain a final coordinates block.
    """
    import numpy as np

    # 1. Parse alat (in Bohr)
    alat_match = re.search(r"lattice parameter \(alat\)\s+=\s+([\d.]+)\s+a\.u\.", output)
    if not alat_match:
        return None
    alat_bohr = float(alat_match.group(1))
    alat_ang  = alat_bohr * BOHR_TO_ANG

    # 2. Parse crystal axes a(1), a(2), a(3) in alat units — use the LAST
    #    occurrence so we get the final relaxed cell, not the initial one
    axes_pattern = re.compile(
        r"crystal axes:.*?"
        r"a\(1\)\s*=\s*\(\s*([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s*\).*?"
        r"a\(2\)\s*=\s*\(\s*([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s*\).*?"
        r"a\(3\)\s*=\s*\(\s*([-\d.]+)\s+([-\d.]+)\s+([-\d.]+)\s*\)",
        re.DOTALL,
    )
    axes_matches = axes_pattern.findall(output)
    if not axes_matches:
        return None

    last = axes_matches[-1]
    lattice = np.array([float(x) for x in last], dtype=float).reshape(3, 3) * alat_ang

    # 3. Parse the final ATOMIC_POSITIONS block
    final_block = re.search(
        r"Begin final coordinates.*?ATOMIC_POSITIONS\s*\(crystal\)(.*?)End final coordinates",
        output,
    ...
    species, coords = [], []
    for line in final_block.group(1).strip().splitlines():
        parts = line.split()
        if len(parts) == 4:
